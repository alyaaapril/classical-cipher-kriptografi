...
window = Tk()
window.title("Special File - Extended Vigenere Cipher")
...
def open_file():
    ...
    print(filename)
    entry_text = readBinaryFile(filename)
...
def encrypt_file():
    ...
    key = entry_key.get()
    encryptBinaryExtendedVigenereCipher(path, key)

def decrypt_file():
    path = entry_message.get()
    key = entry_key.get()
    decryptBinaryExtendedVigenereCipher(path, key)
...
#Back to gui vigenere encription
btn_back = Button(window, width=20, bg="#B8B8C7", text="Back to Text Encryption", command=click_back)
